...
async def crawl_single_page(url: str) -> List[Dict[str, Any]]:
    """Crawls a single web page and returns its markdown content."""
    browser_config = BrowserConfig(
        headless=True, verbose=False
    )  # Added verbose=False for consistency
    crawl_config = CrawlerRunConfig(
        cache_mode=CacheMode.BYPASS, stream=False
    )  # Added cache_mode and stream for consistency
    async with AsyncWebCrawler(config=browser_config) as crawler:
        result = await crawler.arun(url=url, config=crawl_config)
        if result.success and result.markdown:
            return [{"url": url, "markdown": result.markdown}]
...
def main():
    ...
    parser.add_argument(
        "--embedding-model", default="all-MiniLM-L6-v2", help="Embedding model name"
    )
    parser.add_argument(
        "--chunk-size", type=int, default=1000, help="Max chunk size (chars)"
    )
    parser.add_argument(
        "--max-depth", type=int, default=3, help="Recursion depth for regular URLs"
    )
    parser.add_argument(
        "--max-concurrent", type=int, default=10, help="Max parallel browser sessions"
    )
    parser.add_argument(
        "--batch-size", type=int, default=100, help="ChromaDB insert batch size"
    )
    parser.add_argument(
        "--no-recursion",
        action="store_true",
        help="Crawl only the provided URL without following internal links",
    )
    ...
    else:  # This 'else' handles regular URLs
        print(f"Detected regular URL: {url}")
        # NEW: Check for the --no-recursion flag
        if args.no_recursion:
            print("Single page crawl requested (no recursion).")
            crawl_results = asyncio.run(
                crawl_single_page(url)
            )  # Call the new single page function
        else:
            print("Recursive crawl requested.")
            crawl_results = asyncio.run(
    ...
    print(
        f"Inserting {len(documents)} chunks into ChromaDB collection '{args.collection}'..."
    )

    client = get_chroma_client(args.db_dir)
    collection = get_or_create_collection(
        client, args.collection, embedding_model_name=args.embedding_model
    )
    add_documents_to_collection(
        collection, ids, documents, metadatas, batch_size=args.batch_size
    )

    print(
        f"Successfully added {len(documents)} chunks to ChromaDB collection '{args.collection}'."
    )
